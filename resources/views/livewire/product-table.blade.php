<div>
    <div class="bg-[#0a0a0a] border border-gray-900 p-4 rounded-xl mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <input wire:model.live="search" type="text" placeholder="Search by Product Name, SKU, or Seller..." class="md:col-span-2 w-full bg-black border border-gray-800 text-gray-300 rounded-lg px-4 py-2.5 focus:border-[#39FF14] focus:outline-none focus:ring-1 focus:ring-[#39FF14] transition-all text-sm">

        <select wire:model.live="category" class="w-full bg-black border border-gray-800 text-gray-300 rounded-lg px-4 py-2.5 text-sm focus:border-[#39FF14] focus:outline-none">
            ...
            @foreach($products as $product)
                <tr class="group hover:bg-gray-900/30 transition-colors {{ $product->status === 'suspended' ? 'opacity-60' : '' }}">
                    ...
                    <td class="p-5 align-top text-right">
                        ...
                        @else
                            <button wire:click="suspend({{ $product->id }})" class="text-{{ $product->status === 'active' ? 'red' : 'green' }}-500 hover:text-{{ $product->status === 'active' ? 'red' : 'green' }}-400 text-[10px] font-bold uppercase hover:underline">
